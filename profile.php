...
$email=$_POST['emailid'];
$aid=$_SESSION['id'];
$query="update users set email_id=? where id=? ";
...
$ai=$_SESSION['id'];
$udate=date('Y-m-d');
	$sql="SELECT upassword FROM users where upassword=?";
	$chngpwd = $mysqli->prepare($sql);
	$chngpwd->bind_param('si',$op,$aid);
	$chngpwd->execute();
	$chngpwd->store_result(); 
    $row_cnt=$chngpwd->num_rows;
	if($row_cnt>0)
	{
		$con="update users set upassword=? where id=?";
									...
									<div class="panel-body">
										<form method="post" class="form-horizontal">
											...
											<div class="form-group">
												...
												<div class="col-sm-10">
													<input type="text" value="<?php echo $row->uname;?>" disabled class="form-control"><span class="help-block m-b-none">
													Username can't be changed.</span> </div>
											</div>
											<div class="form-group">
												<label class="col-sm-2 control-label">Email</label>
												<div class="col-sm-10">
	<input type="email" class="form-control" name="emailid" id="emailid" value="<?php echo $row->email_id;?>" required="required">
